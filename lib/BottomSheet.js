...
class BottomSheet extends React.PureComponent<Props> {
  ...
  renderOption = (options: Array<Object>) => {
    return options.map((item, index) => {
      return (
        <View style={[this.props.grid ? styles.itemContainerGrid : styles.itemContainer]} key={index}>
          <TouchableNativeFeedback
            onPress={item.onPress}
            background={TouchableNativeFeedback.SelectableBackground()}>
            <View style={[this.props.grid ? styles.itemGrid : styles.item]}>
              {item.icon}
              <Text
                style={[styles.text, {fontFamily: this.props.fontFamily}, this.props.grid ? styles.textGrid : null]}>
                {item.title}
              </Text>
            </View>
          </TouchableNativeFeedback>
          {this.props.itemDivider === index + 1 && !this.props.grid ? (
    ...
    return (
      <Modal
        ...
        <ScrollView style={styles.modal}>
          {this.renderTitle()}
          <View style={[this.props.grid ? styles.gridContainer : null]}>
